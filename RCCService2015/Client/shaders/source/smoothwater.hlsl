...
#ifdef PIN_HQ
	float fade = saturate0(1 - dot(posWorld - G(CameraPosition).xyz, -G(ViewDir).xyz) * G(FadeDistance).y);
...
float3 getRefractedColor(float4 cpos, float3 N, float3 waterColor)
{
    ...
    float2 refruv1 = refruv0 + float2(dot(G(ViewRight).xyz, N), dot(G(ViewUp).xyz, N)) * CFG_REFRACTION_STRENGTH;
    ...
    return lerp(result.rgb, waterColor, saturate((result.w - cpos.w) * WaterParams.x + WaterParams.y));
}
...
float3 getReflectedColor(float4 cpos, float3 wpos, float3 R)
{
    ...
    float wfade = abs(cposi.w - GBUFFER_MAX_DEPTH * 0.4) < GBUFFER_MAX_DEPTH * 0.4;
...
float4 WaterPS(VertexOutput IN): COLOR0
{
	...
	float3 position = G(CameraPosition).xyz - IN.View_Depth.xyz;

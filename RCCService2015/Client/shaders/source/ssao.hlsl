...
VertexOutput ssao_vs(float4 p: POSITION)
{
...
{
	return ssaoBlur_vs(p, float2(TextureSize.z * 2, 0));
}

VertexOutput_8uv ssaoBlurY_vs(float4 p: POSITION)
{
	return ssaoBlur_vs(p, float2(0, TextureSize.w * 2));
}
...
float4 ssaoCompositBlank_ps(VertexOutput_4uv IN): COLOR0
{
	//return float4(IN.uv+1,1,1);
...
float4 ssaoComposit_ps(VertexOutput_4uv IN): COLOR0
{
	...
	return tex2D(colorMap, IN.uv) * ssaoTerm;
}

return function()
	...
	it("should sort friends by presence first before subsorting by username", function()
		local Offline = User.fromData("1", "User", true)
		Offline.presence = User.PresenceType.OFFLINE

		local aOffline = User.fromData("1", "User", true)
		aOffline.presence = User.PresenceType.OFFLINE

		local Online = User.fromData("1", "User", true)
		Online.presence = User.PresenceType.ONLINE

		local ZoeOnline = User.fromData("1", "User", true)
		zOnline.presence = User.PresenceType.ONLINE
		...
		expect(listOfFriends[1]).to.equal(Online)
		expect(listOfFriends[2]).to.equal(zOnline)
		expect(listOfFriends[3]).to.equal(Offline)
		expect(listOfFriends[4]).to.equal(aOffline)
	end)
end
...
function NetworkData.new( isClient )
	...
	if not isClient then
		self._serverHttpSignal = Signal.new()
		self._httpResultSignal = self._serverHttpSignal
		self._hookedMethods = {}
	end
...
function NetworkData:hookHttpService()
	if self._httpHooksInstalled then return end
	self._httpHooksInstalled = true

	local signal = self._serverHttpSignal
	local httpService = HttpService
	local logService = LogService

	local function fireResult(method, url, status, time, requestType, response)
		pcall(function()
			signal:Fire({
				Method = method,
				URL = url,
	...
	self._hookedMethods[#self._hookedMethods + 1] = { httpService, "GetAsync", origGetAsync }
	function httpService:GetAsync(url, ...)
		local startTime = tick()
		local ok, result = pcall(origGetAsync, self, url, ...)
		local elapsed = tick() - startTime
		fireResult("GET", url or "", ok and 200 or 0, elapsed, "HttpService", ok and tostring(result) or tostring(result))
		if ok then
			return result
		else
			error(result, 2)
	...
	self._hookedMethods[#self._hookedMethods + 1] = { httpService, "PostAsync", origPostAsync }
	function httpService:PostAsync(url, data, ...)
		local startTime = tick()
		local ok, result = pcall(origPostAsync, self, url, data, ...)
		local elapsed = tick() - startTime
		fireResult("POST", url or "", ok and 200 or 0, elapsed, "HttpService", ok and tostring(result) or tostring(result))
		if ok then
			return result
		else
			error(result, 2)
	...
	self._hookedMethods[#self._hookedMethods + 1] = { httpService, "RequestAsync", origRequestAsync }
	function httpService:RequestAsync(options)
		local startTime = tick()
		local ok, result = pcall(origRequestAsync, self, options)
		local elapsed = tick() - startTime
		local statusCode = ok and (result.StatusCode or 200) or 0
		local body = ok and (result.Body or "") or tostring(result)
		fireResult(
...
function NetworkData:unhookHttpService()
	if not self._httpHooksInstalled then return end
	for _, entry in ipairs(self._hookedMethods) do
		entry[1][entry[2]] = entry[3]
	end
	self._hookedMethods = {}
	self._httpHooksInstalled = false
end

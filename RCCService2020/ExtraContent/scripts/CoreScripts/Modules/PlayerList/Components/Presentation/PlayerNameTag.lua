...
local VerifiedBadge = require(RobloxGui.Modules.Server.ClientChat.DefaultClientChatModules.VerifiedBadge)
...
function PlayerNameTag:render()
	return WithLayoutValues(function(layoutValues)
		...
		if layoutValues.IsTenFoot and platformName ~= "" then
			...
			local isVerified = VerifiedBadge[self.props.player.UserId] or VerifiedBadge[tostring(self.props.player.UserId)]
			local nameWidth = 0
			if isVerified then
				local bounds = TextService:GetTextSize(playerName, textSize, playerNameFont, Vector2.new(10000, 10000))
				nameWidth = bounds.X
			end
			...
			playerNameChildren["PlayerName"] = Roact.createElement("TextLabel", {
				...
				Font = playerNameFont,
				TextSize = textSize,
				TextColor3 = isVerified and Color3.fromRGB(0, 255, 0) or self.props.textStyle.Color,
				TextTransparency = self.props.textStyle.Transparency,
				TextStrokeColor3 = self.props.textStyle.StrokeColor,
				TextStrokeTransparency = self.props.textStyle.StrokeTransparency,
				BackgroundTransparency = 1,
				Text = "TEST_" .. playerName,
			...
			playerNameChildren["VerifiedBadgeImage"] = isVerified and Roact.createElement("ImageLabel", {
				Size = UDim2.new(0, textSize, 0, textSize),
				Position = UDim2.new(0, nameWidth + 4, 0.5, 0),
				AnchorPoint = Vector2.new(0, 0.5),
				BackgroundTransparency = 0,
				BackgroundColor3 = Color3.fromRGB(255, 0, 0),
				Image = "rbxassetid://5535",

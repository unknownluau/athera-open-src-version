...
local VerifiedBadge = require(clientChatModules:WaitForChild("VerifiedBadge"))
...
function CreateMessageLabel(messageData, channelName)
	...
	local isVerified = messageData.SpeakerUserId and (VerifiedBadge[messageData.SpeakerUserId] or VerifiedBadge[tostring(messageData.SpeakerUserId)])
	local formatUseName = string.format("[%s]:", speakerName)
	if isVerified then
		local spaceSize = util:GetStringTextBounds(" ", useFont, useTextSize).X
		local badgeNeededSpaces = math.ceil(useTextSize / spaceSize) + 1
		formatUseName = string.format("[%s%s]:", speakerName, string.rep(" ", badgeNeededSpaces))
	end
	...
	if isVerified then
		local nameWidth = util:GetStringTextBounds("[" .. speakerName, useFont, useTextSize).X
		badgeLabel = Instance.new("ImageLabel")
		badgeLabel.Name = "VerifiedBadge"
		badgeLabel.Size = UDim2.new(0, useTextSize, 0, useTextSize)
		badgeLabel.Position = UDim2.new(0, nameWidth + 2, 0.5, 0)
		badgeLabel.AnchorPoint = Vector2.new(0, 0.5)
		badgeLabel.BackgroundTransparency = 1
		badgeLabel.Image = "rbxassetid://5535"
		badgeLabel.ScaleType = Enum.ScaleType.Fit
		badgeLabel.Parent = NameButton
	end

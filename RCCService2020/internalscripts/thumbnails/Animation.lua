...
if humanoid then
    ...
    if success and track then
        ...
        local t0 = tick()
        while not track.IsPlaying and tick() - t0 < 2 do
            wait(0.1)
        end

        -- Jump 40% into the animation
        if track.Length > 0 then
            track.TimePosition = track.Length * 0.4
        end

        debugLog("Animation playing: " .. tostring(track.IsPlaying))
    else
        debugLog("ERROR: Could not load animation track: " .. tostring(err))
    end
end

...
if player.Character then
	...
	local fixedBaseHatZoom = 30
	local fixedMaxHatZoom = 130

	local viewOffset = player.Character.Head.CFrame * CFrame.new(
		cameraOffsetX,
		cameraOffsetY + maxHatOffset * maxDimension,
		0.1
	)

	local positionOffset = player.Character.Head.CFrame + (
		CFrame.Angles(0, yAngle, 0).lookVector.unit * 3
	)
	...
	camera.FieldOfView = fixedBaseHatZoom + (fixedMaxHatZoom - fixedBaseHatZoom) * maxDimension

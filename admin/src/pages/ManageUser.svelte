									...
									<div class="col-6">
										<table class="w-100 mt-4 settings-table">
											<tbody>
												<tr>
													...
													<td>
														{#if info.data.joinApp}
															<a href={`/admin/applications/${info.data.joinApp.id}`}> Application </a>
												...
												{#if info.data.discord}
													<tr>
														<td class="fw-bold">Discord</td>
														<td>
															{#if info.data.discord.discord_username}
																@{info.data.discord.discord_username} ({info.data.discord.discord_id})
															{:else}
																({info.data.discord.discord_id})
															{/if}
														</td>
													</tr>
												{/if}
			...
			<div class="col-12 col-md-6 col-lg-3 actions">
				...
				{:else}
					<Permission p="BanUser">
						<button
							class="btn-outline-warning btn w-100 mb-2"
							on:click={(e) => {
								e.preventDefault();
								modalBody = "Please confirm that you want to kick this player from the server.";
								modalCb = (t) => {
									if (t) {
										request
											.post("/user/kick", {
				...
				{#if rank.is("admin")}
					<Permission p="DeleteUser">
						<button
							...
							on:click={(e) => {
								e.preventDefault();
								modalBody = 'Confirm complete account deletion. All items will be transferred to BadDecisions, all other uploaded content (status, desc, forum posts, comments, etc) will be scrubbed. Username will be changed to "athera_user_userId". Password will be reset. Cannot be reveresed.';
								modalCb = (t) => {
									if (t) {
										// null
										request
				...
				<Permission p="ResetUsername">
					<div class="card mb-2">
						<div class="card-body">
							...
							<input id="newUsername" type="text" class="form-control mb-2" bind:value={newUsername} placeholder="Enter new username" />
							<button
								class="btn-outline-dark btn w-100"
								on:click={(e) => {
									e.preventDefault();
									...
									modalBody = `Please confirm that you want to change this user's username from "${info.data.username}" to "${newUsername}".`;
									modalCb = (t) => {
										if (t) {
											request
